      ...
      <main className="max-w-screen-2xl mx-auto px-4 py-5">
        ...
        <div className="card p-3 mb-4">
          <div className="flex items-center gap-3 mb-3 pb-3 border-b border-white/[0.06]">
            ...
            {mode==='live' && <span className="flex items-center gap-1.5 text-xs text-emerald-400"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse-dot"/>Connected to real cloud logs</span>}
            {mode==='simulate' && <span className="text-xs text-slate-600">Switch to Live when cloud credentials are configured</span>}
          </div>
          {mode==='simulate' ? <SimBar apiUrl={API} onDone={refresh} /> : <LiveStatus apiUrl={API} />}
        </div>
