const SOURCE_BADGE = {
  ...
  direct : 'border-white/[0.08]  bg-white/[0.04]  text-slate-400',
}
...
export default function EventStream({ events }) {
  if (!events.length) return (
    <div className="text-center py-16 text-slate-600">
      ...
      <p className="text-sm">No events yet — simulate or upload logs</p>
    </div>
  )
      ...
      {events.map((e, i) => (
        <div key={e.id||i} className="card p-3 card-hover">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <span className={`badge border ${e.stream_type==='HLS' ? 'border-emerald-500/25 bg-emerald-500/10 text-emerald-300' : 'border-blue-500/25 bg-blue-500/10 text-blue-300'} text-[10px] font-bold`}>
              {e.stream_type||'HLS'}
            </span>
            {e.source && e.source !== 'direct' && (
              <span className={`badge border ${SOURCE_BADGE[e.source]||SOURCE_BADGE.direct} text-[10px] font-bold`}>
                {e.source?.toUpperCase()}
              </span>
            )}
            <span className="text-[10px] text-slate-600 ml-auto font-mono">
              {e.timestamp ? new Date(e.timestamp+'Z').toLocaleTimeString() : ''}
            </span>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {e.registered_country && e.geo && e.registered_country !== e.geo && (
              <span className="badge border border-yellow-500/25 bg-yellow-500/10 text-yellow-400 text-[9px]">⚠ Geo</span>
            )}
          </div>
