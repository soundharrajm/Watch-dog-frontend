...
// All authenticated API calls use Bearer token in header — never in URL
const authFetch = (url, sessionTok, opts = {}) =>
  apiFetch(url, {
    ...opts,
    headers: {
  ...
  return (
    <label className="flex items-center gap-2.5 cursor-pointer select-none">
      <div onClick={() => onChange(!checked)}
        className={`relative w-9 h-5 rounded-full transition-colors duration-200 ${checked ? 'bg-violet-600' : 'bg-white/10'}`}>
...
function SaveBtn({ onClick, saving, saved }) {
  return (
    <button onClick={onClick} disabled={saving}
      className={`btn px-4 py-2 text-sm font-bold border-none transition-all
        ${saved ? 'bg-emerald-600 text-white' : saving ? 'bg-violet-600/40 text-white cursor-not-allowed' : 'bg-violet-600 hover:bg-violet-500 text-white'}`}>
...
export default function Settings({ apiUrl: initUrl, onClose, onUrlSaved }) {
  ...
  const [sessionTok, setSessionTok] = useState('')   // Bearer token — in memory only
  ...
  const authenticate = async () => {
    ...
    try {
      clearKeyCache()
      // Encrypt secret with RSA-OAEP + AES-256-GCM
      ...
      // Fetch config using Bearer token in header — not URL
      const c = await authFetch(`${API}/config/`, tok).then(r => r.json())
      setCfg(c)
  ...
  const testBackend = async () => {
    ...
    try {
      const r = await apiFetch(`${backendUrl}/health`, { signal: AbortSignal.timeout(5000) })
      setTestOk(r.ok)
  ...
  // ── Save cloud — ENCRYPTED payload ───────────────────────────────────────
  const saveCloud = async (p) => {
    markSaving(p)
    try {
      // Encrypt cloud credentials before sending
      const encrypted = await encryptPayload({ provider: p, config: cfg.clouds[p] }, API)
      const r = await authFetch(`${API}/config/cloud`, sessionTok, {
        method : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body   : JSON.stringify(encrypted),
      })
      r.ok ? markSaved(p) : markSaving(p, false)
    } catch { markSaving(p, false) }
  }

  // ── Save detection — plain JSON (no sensitive data) ───────────────────────
  const saveDet = async () => {
    markSaving('det')
    const r = await authFetch(`${API}/config/detection`, sessionTok, {
      method : 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  // ── Change secret — ENCRYPTED payload ────────────────────────────────────
  const changeSecret = async () => {
    setSecretErr(''); markSaving('secret')
    try {
      // Encrypt new secret before sending
      const encrypted = await encryptPayload({ new_secret: newSecret }, API)
      const r = await authFetch(`${API}/config/change-secret`, sessionTok, {
        method : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body   : JSON.stringify(encrypted),
      })
      if (!r.ok) { setSecretErr('Failed to update secret'); markSaving('secret', false); return }
      setNewSecret('')
      markSaved('secret')
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
      ...
      <div className="bg-[#12121f] border border-white/[0.1] rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07]">
          ...
          <button onClick={onClose} className="w-7 h-7 rounded-lg border border-white/[0.1] text-slate-500 hover:text-slate-300 flex items-center justify-center">✕</button>
        </div>
          ...
          {TABS.map((t, i) => (
            <button key={t} onClick={() => setTab(i)} disabled={i > 0 && !authed}
              className={`px-4 py-3 text-xs font-semibold transition-all border-b-2 -mb-px
                ${tab===i ? 'border-violet-500 text-violet-300' : 'border-transparent text-slate-500 hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed'}`}>
              {t}
            </button>
          ))}
        </div>
            ...
            <>
              ...
              <div className="flex gap-2 mb-4">
                <input value={backendUrl} onChange={e => setBackendUrl(e.target.value)}
                  placeholder="http://localhost:8001" className="inp flex-1 mb-0"/>
                <button onClick={testBackend} disabled={testing}
                  className={`btn flex-shrink-0 ${testOk===true?'border-emerald-500/30 text-emerald-400':testOk===false?'border-red-500/30 text-red-400':'border-white/[0.1] text-slate-400'}`}>
                  ...
                  <>
                    <div className="flex gap-2">
                      <input type="password" value={secret} onChange={e => setSecret(e.target.value)}
                        ...
                        placeholder="Admin secret" className="inp flex-1 mb-0"/>
                    ...
                    <div className="mt-3 flex items-center gap-2 text-[10px] text-slate-600">
                      ...
                      Secret encrypted with RSA-OAEP + AES-256-GCM before sending
                    </div>
                  </>
                ) : (
                  <>
                    <div className="px-3 py-2.5 rounded-lg border border-emerald-500/25 bg-emerald-500/5 text-sm text-emerald-400 mb-4">
                      ✓ Unlocked — session token active (1hr TTL)
                    </div>

                    {/* Encryption status */}
                    <div className="mb-4 p-3 rounded-lg border border-violet-500/20 bg-violet-500/5">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-violet-400 mb-2">Encryption status</p>
                      <div className="space-y-1 text-[11px] text-slate-500">
                        <div className="flex items-center gap-2"><span className="text-emerald-400">✓</span> Auth secret — RSA-OAEP + AES-256-GCM</div>
                        <div className="flex items-center gap-2"><span className="text-emerald-400">✓</span> Cloud credentials — RSA-OAEP + AES-256-GCM</div>
                        <div className="flex items-center gap-2"><span className="text-emerald-400">✓</span> New secret — RSA-OAEP + AES-256-GCM</div>
                        <div className="flex items-center gap-2"><span className="text-emerald-400">✓</span> Session token — Authorization: Bearer header</div>
                        <div className="flex items-center gap-2"><span className="text-slate-600">—</span> Detection rules — plain JSON (no sensitive data)</div>
                      </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-white/[0.06]">
                      ...
                      <div className="flex gap-2">
                        <input type="password" value={newSecret} onChange={e => setNewSecret(e.target.value)}
                          onKeyDown={e => e.key==='Enter' && newSecret.trim() && changeSecret()}
                          placeholder="New secret" className="inp flex-1 mb-0"/>
                      ...
                      <p className="text-[10px] text-slate-600 mt-1.5">Encrypted before sending · Only current admin can change</p>
                    </div>
                  </>
                )}
              </div>
          ...
          {tab === 1 && cfg && ['gcp','aws','azure','cdn'].map(p => (
            <div key={p} className={sec}>
              ...
              {p==='gcp' && <>
                ...
                <p className="text-[10px] text-emerald-600 mt-2 mb-3">🔒 Credentials encrypted before sending</p>
              </>}
              ...
              {p==='aws' && <>
                ...
                <p className="text-[10px] text-emerald-600 mb-3">🔒 Credentials encrypted before sending</p>
              </>}
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {(cfg.detection.suspicious_agents||[]).map(a => (
                  <span key={a} className="badge border border-red-500/25 bg-red-500/10 text-red-400 font-mono">
                    {a}
                    <button onClick={() => setDet('suspicious_agents', cfg.detection.suspicious_agents.filter(x=>x!==a))} className="ml-1 text-slate-500 hover:text-red-400">✕</button>
                  </span>
                ))}
              </div>
              <div className="flex gap-2 mb-4">
                <input value={newAgent} onChange={e=>setNewAgent(e.target.value)}
                  onKeyDown={e=>{if(e.key==='Enter'&&newAgent.trim()){setDet('suspicious_agents',[...(cfg.detection.suspicious_agents||[]),newAgent.trim()]);setNewAgent('')}}}
                  placeholder="Add keyword e.g. hlsdump" className="inp flex-1 mb-0"/>
                <button onClick={()=>{if(newAgent.trim()){setDet('suspicious_agents',[...(cfg.detection.suspicious_agents||[]),newAgent.trim()]);setNewAgent('')}}}
                  className="btn border-violet-500/30 text-violet-400 hover:bg-violet-500/10">+ Add</button>
              </div>
              <div className="mb-3 text-[10px] text-slate-600">Detection rules contain no sensitive data — sent as plain JSON</div>
