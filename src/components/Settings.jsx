...
export default function Settings({ apiUrl: initUrl, onClose, onUrlSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={e => e.target===e.currentTarget && onClose()}>
      <div className="bg-surface border border-white/[0.1] rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
          ...
          {tab === 0 && (
            <>
              ...
              <div className="border-t border-white/[0.06] pt-4">
                <p className={label}>Admin unlock</p>
                {!authed ? (
                  <>
          ...
          {tab === 1 && cfg && ['gcp','aws','azure','cdn'].map(p => (
            <div key={p} className={sec}>
              ...
              {p === 'gcp' && <>
                ...
                <label className={label}>Log filter</label>
          ...
          {tab === 2 && cfg && (
            <>
              <div className="grid grid-cols-2 gap-3 mb-4">
                {[['Max concurrent IPs','max_concurrent_ips'],['Max tokens / IP (30s)','max_tokens_per_ip'],['Dedup window (sec)','dedup_window_sec'],['Spray window (sec)','token_spray_window']].map(([lbl,key]) => (
                  <div key={key}>
                    <label className={label}>{lbl}</label>
                    <input type="number" value={cfg.detection[key]||''} onChange={e => setDet(key,+e.target.value)} className="inp" />
                  </div>
                ))}
              </div>

              <label className={label}>Suspicious user-agents</label>
