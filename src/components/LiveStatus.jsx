...
function StatusDot({ status }) {
  if (status === 'ok' || status === 'running')
    return <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse-dot inline-block" />
  if (status === 'error')
    return <span className="w-2 h-2 rounded-full bg-red-400 animate-pulse-dot inline-block" />
  if (status === 'degraded' || status === 'misconfigured')
...
export default function LiveStatus({ apiUrl }) {
  ...
  return (
    <div>
      {/* Overall status bar */}
      <div className={`flex items-center gap-3 px-3 py-2 rounded-lg border mb-4
        ${overall === 'ok'       ? 'border-emerald-500/25 bg-emerald-500/5' :
          overall === 'degraded' ? 'border-yellow-500/25 bg-yellow-500/5'  :
          overall === 'error'    ? 'border-red-500/25    bg-red-500/5'     :
                                   'border-white/[0.07]  bg-white/[0.02]'  }`}>
        <StatusDot status={overall} />
        <span className={`text-xs font-bold uppercase
          ${overall === 'ok' ? 'text-emerald-400' : overall === 'degraded' ? 'text-yellow-400' : overall === 'error' ? 'text-red-400' : 'text-slate-500'}`}>
          System {overall}
        </span>
          ...
          return (
            <div key={key} className="card p-3">
              ...
              <div className="text-[10px] text-slate-600 mt-1 space-y-0.5">
                ...
                {key === 'engine'    && check.suspicious_agents && <p>{check.suspicious_agents} agent rules · Max {check.max_concurrent_ips} IPs</p>}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
        {Object.entries(CLOUD_INFO).map(([key, info]) => {
          ...
          const status   = consumer.status || 'stopped'
          const isLive   = status === 'running'
          const isError  = status === 'error' || status === 'misconfigured'
          return (
            <div key={key} className={`rounded-xl border p-3 transition-all
              ${isLive  ? `${info.bg} ${info.border}` :
              ...
              <p className={`text-xs font-semibold ${isLive ? info.color : isError ? 'text-red-400' : 'text-slate-600'}`}>
                {info.label}
              </p>
              <p className="text-[10px] text-slate-600 mt-1">
                {isLive  ? 'Streaming live logs' :
                 isError ? `Missing: ${consumer.missing?.join(', ') || 'credentials'}` :
                 status === 'disabled' ? 'Disabled in settings' :
                 'Configure in Settings → Clouds'}
              </p>
            </div>
          )
        })}
      ...
      {Object.values(consumers).every(c => ['stopped','disabled'].includes(c.status)) && (
        <div className="mt-3 px-4 py-3 rounded-lg border border-yellow-500/20 bg-yellow-500/5 text-xs text-yellow-400">
          ⚠ No live consumers running — go to <strong>⚙ Settings → Clouds</strong> and add credentials to stream real logs
        </div>
      )}
    </div>
  )
