...
const THREAT = {
  CRITICAL : { dot:'bg-red-500 shadow-red-500/50',    text:'text-red-400',    ring:'border-red-500/30 bg-red-500/10'    },
  HIGH     : { dot:'bg-orange-500 shadow-orange-500/50', text:'text-orange-400', ring:'border-orange-500/30 bg-orange-500/10' },
  MEDIUM   : { dot:'bg-yellow-500 shadow-yellow-500/50', text:'text-yellow-400', ring:'border-yellow-500/30 bg-yellow-500/10' },
...
export default function TopBar({ threat, liveAlerts, apiUrl, onOpenSettings }) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-base/90 backdrop-blur-md border-b border-white/[0.07]">
      <div className="max-w-screen-2xl mx-auto px-4 h-14 flex items-center gap-3">
        ...
        {liveAlerts.length > 0 && (
          <div className="relative">
            <button onClick={() => setShowLive(v => !v)} className="relative flex items-center gap-1.5 btn border-red-500/30 bg-red-500/10 text-red-400 hover:bg-red-500/20">
              <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse-dot" />
              {liveAlerts.length} live
            </button>
            {showLive && (
              <div className="absolute right-0 top-[calc(100%+8px)] w-80 card border-red-500/20 bg-surface shadow-2xl z-50">
        ...
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-bold ${t.ring} ${t.text}`}>
          <span className={`w-2 h-2 rounded-full shadow-lg ${t.dot}`} />
          Threat: {threat}
        </div>
